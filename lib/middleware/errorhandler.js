(function() {
  ...
  module.exports = function(globals) {
    var ExpectedError, FatalError, ForbiddenError, HandledInternalError, InternalError, InvalidArgumentsError, NotFoundError, ValidationError, app, handleBadRequest, handleError, handleExpectedError, handleFatalError, handleForbidden, handleHandledInternalError, handleInternalError, handleInvalidArgumentsError, handleNotAuthorized, handleNotFound, handleValidationError, logger;
    ...
    handleError = function(err, req, res, next) {
      ...
      } else if (err instanceof InvalidArgumentsError) {
        return handleInvalidArgumentsError(err, req, res, next);
      } else {
    ...
    handleInvalidArgumentsError = function(err, req, res, next) {
      logger.warn({
        message: "Invalid Arguments Error on path=" + req.path + " with message=" + err.message,
        ctx: res.locals.ctx
      });
      if (req.accepts(['html', 'json']) === 'json') {
        return res.json(500, {
